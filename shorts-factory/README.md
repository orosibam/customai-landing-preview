...
- 1688 도 상품 상세 HTML 에 영상 주소가 그대로 박혀 있다.

다만 **어느 링크를 열지**는 이 방법으로 못 정한다 — 그건 아래 「키워드 조준」 항목이다.

node 에는 크롬 TLS 지문을 흉내 내는 검증된 수단이 없어서 그 조각만 파이썬 프로세스로
떼어내고 `src/lib/scrapers/cn-bridge.ts` 가 호출한다.

### 키워드 조준은 사람이 한 번 해야 한다

**두 플랫폼 다 비로그인으로는 키워드 검색이 안 된다.** 이건 우회할 방법을 못 찾은 게
아니라 전부 재보고 나온 결론이다.

| 방법 | 결과 |
|---|---|
| 로그인 세션으로 검색 결과 href 수확 | ✅ **유일하게 검증됨. 키워드 조준이 된다** |
| 앱에서 공유한 링크 | 토큰 포함되나 사람이 하나씩 해야 함 |
| 구글 `site:` 검색 | 인덱싱된 링크엔 보통 토큰이 없음 |
...
**재고 소진**이라 점검 출력에서도 `[재고없음]` 으로 따로 표시한다 — 섞어놓으면 있지도 않은
버그를 찾게 된다.

### ⚠️ href 를 자르지 않는다

`xsec_token` 은 게시물마다 다르고 **쿼리스트링에 붙어 있다.** note id 만 남기고 토큰을
버리면 그 링크는 전부 404 다. 실제로 그렇게 **55건을 통째로 날린 적이 있다** — 같은 55건인데
토큰을 남긴 쪽은 쓸 수 있고 지운 쪽은 못 쓴다.

그래서 이 코드는 URL 을 한 글자도 건드리지 않는다. `xhsNote(url)` / `aliOffer(url)` 은
id+토큰이 아니라 **URL 통째로** 받는다. 재조립하면 내가 아는 파라미터만 다시 붙게 되고
모르는 건 조용히 사라지기 때문이다. DB 의 `unique` 도 URL 전체로 건다.

### 되는 것 / 안 되는 것

| | 되는 것 | 안 되는 것 |
|---|---|---|
| 샤오홍슈 | 수확한 URL 로 노트 상세(영상·좋아요·수집), `/explore` 피드(접근 확인용) | **키워드 검색**(로그인 벽), 계정별 노트 목록, 피드로 조준 |
| 1688 | 홈·상세 페이지 | **상품 검색** — 25KB 를 받지만 상품 데이터가 HTML 에 없다 |
...
| 샤오홍슈 피드 | 1회 호출에 **27건**. 반복해도 무작위라 조준 불가 (980건에 적중 0건) |
...
껍데기이며 JS 가 나중에 목록을 받아온다. 그래서 HTTP 만으로는 1688 검색을 뚫을 수 없고,
상세 URL 을 수확해 오는 경로(위)로 간다.
