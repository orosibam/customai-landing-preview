...
/**
 * 편집자가 **고를 수 있게** 미리 받아두는 소재 개수.
 *
 * TARGET_SOURCE_COUNT 와 다른 값인 이유가 중요하다. 5는 완성 영상에 실제로 쓰는
 * 개수고, 이건 그 5개를 고르기 위해 확보해두는 후보의 수다.
 *
 * 둘을 같게 뒀더니 문제가 있었다. 설계도 컷이 6개인데 소재도 5개면 배정이 사실상
 * 1:1 이 되어 **고르는 게 아니라 채우는 게 된다.** 설계도가 "뚜껑을 눌러 닫는다"
 * 라고 적어놔도 그 장면이 있는 소재가 없으면 아무거나 들어간다. 편집자가 프레임을
 * 눈으로 보고 action 에 맞는 걸 고르도록 만들어놨는데 고를 대상이 없었던 것이다.
 *
 * 후보가 많으면 부수 효과도 있다. 소스가 많을수록 소스당 사용 길이가 짧아져
 * MAX_CLIP_SEC 상한에 여유가 생긴다.
 *
 * 올리면 편집자에게 넘기는 프레임 수가 늘어 비용이 오른다. editor 가 소재 수에
 * 따라 소재당 프레임 수를 줄여 총량을 맞춘다.
 */
export const FOOTAGE_POOL_SIZE = Number(optionalEnv('FOOTAGE_POOL_SIZE', '12'));
