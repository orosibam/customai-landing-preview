 ...
 * 순서대로 시도하고, 그래도 없으면 타오바오 상품 상세 영상으로 내려간다.
 */
...
export const sourcer: TeamMember = {
  ...
  async work(brief: Brief): Promise<Brief> {
      ...
      const row = await must(
        ...
        db()
          ...
          .insert({
            ...
            source_site: route.includes('타오바오') ? 'taobao' : 'tiktok',
            storage_path: remote,
    ...
    return withNote(
      {
        ...brief,
        footage: { assetIds, localPaths: keptPaths, sourceUrls, triedKeywords },
      },
      'sourcer',
      `소재 ${assetIds.length}개 확보 (${route}). 검색어 "${triedKeywords.join(' → ')}"`,
      route.includes('타오바오')
        ? '틱톡에서 충분히 못 구해 타오바오로 보강했습니다. 화면 톤이 섞일 수 있습니다.'
        : undefined,
    );
  },
