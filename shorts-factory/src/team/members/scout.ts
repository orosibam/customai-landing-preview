 ...
 * ## 같은 제품이어야 한다
 *
 * 한때 "구조만 빌리면 된다" 로 열어뒀다. 화면은 수확한 판매자 영상으로 채우니
 * 릴스에서는 구조만 가져오면 된다는 논리였다. **틀렸다.**
 *
 * 그렇게 만든 첫 완성본을 사용자가 보고 바로 짚었다 — "제품도 해외에서 터진
 * 제품이 아니잖아". 맞는 말이다. 이 파이프라인의 전제는 「해외에서 터진 릴스 →
 * 거기 나온 제품」이다. 릴스와 제품이 남남이면 남는 건 **아무 제품에 아무
 * 구조를 씌운 것**이고, "이미 검증된 걸 수입한다" 가 성립하지 않는다.
 *
 * 그래서 같은 제품이 나오는 릴스만 붙인다. 없으면 붙이지 않고 던진다 —
 * 그 제품은 애초에 이 파이프라인이 다룰 대상이 아니라는 뜻이다.
 */
...
이 상품이 **실제로 나오는** 릴스를 하나 골라라.

이 파이프라인의 전제는 「해외에서 터진 릴스 → 거기 나온 제품」이다. 릴스와 제품이
남남이면 아무 제품에 아무 구조를 씌운 것이 되고, "이미 검증된 걸 수입한다" 가
성립하지 않는다.

- 같은 제품이거나, 이름만 다를 뿐 **같은 물건**이어야 한다.
- 비슷한 장르(둘 다 청소용품 같은)는 **같은 제품이 아니다.** 고르지 마라.
- 확신이 없으면 index 를 null 로 해라. **억지로 고르는 것보다 없다고 하는 게 낫다.**

캡션이 비어 있어 내용을 알 수 없으면 그것도 null 이다 — 추측으로 고르면
그 판단이 나중에 성과 집계를 통째로 오염시킨다.
...
{"index":0,"same_product":true,"why":"한 문장"}`,
  ...
  if (!source || !choice.same_product) {
    throw new HandoffError(
      'scout',
      `"${product.title_ko}" 가 나오는 릴스를 못 찾았습니다 (후보 ${found.length}건 검토).\n` +
        `   판단: ${choice.why}\n` +
        `   다른 제품의 릴스로 대신하지 않습니다 — 이 파이프라인은 "터진 릴스에 나온 제품" 이\n` +
        `   전제라, 릴스와 제품이 남남이면 검증된 걸 수입하는 게 아니라 아무 제품에\n` +
        `   아무 구조를 씌우는 것이 됩니다.`,
      true,
    );
  }
  ...
  console.log(`원본 릴스 재부착: ${source.url} — ${choice.why}`);
